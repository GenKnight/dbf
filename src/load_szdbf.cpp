...
load_sz_dbf::load_sz_dbf(dbf_local_info& m_dbf_param)
...
load_sz_dbf::~load_sz_dbf()
{

}

void load_sz_dbf::init_field_info_sys()
{
	file_info m_sz_file;
	int npos;
	for (size_t i = 0; i < m_variable_field.size(); i++) {
		npos = m_variable_field[i].npos;
		m_sz_file.filedstr = m_dbf_info.my_dbf_index[npos].addr;
		m_sz_file.type = m_dbf_info.my_dbf_index[npos].type;
		if (npos == m_dbf_info.fieldnum - 1) {
			m_sz_file.filedsize = m_dbf_info.m_dbf_head.reclen - m_dbf_info.my_dbf_index[npos].addr;
		} else {
			m_sz_file.filedsize = m_dbf_info.my_dbf_index[npos + 1].addr - m_dbf_info.my_dbf_index[npos].addr;
		}
		m_map_file_info[m_variable_field[i].filename] = m_sz_file;
	}
}

void load_sz_dbf::load_data_to_json(char* output, const char* str)
{
	char* pstr, *epstr;
	//comm value
	{
